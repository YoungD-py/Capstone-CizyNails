...
<body class="bg-gray-50">
    ...
    <div class="max-w-2xl mx-auto p-8">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form method="POST" action="{{ route('admin.types.store') }}" class="space-y-6">
                @csrf

                <!-- Type Name -->
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Type Name *</label>
                    <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-600" required>
                </div>

                <!-- Description -->
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                    <textarea id="description" name="description" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-600">{{ old('description') }}</textarea>
                </div>

                <!-- Staff Count -->
                <div>
                    <label for="staff_count" class="block text-sm font-medium text-gray-700 mb-2">Staff Count *</label>
                    <input type="number" id="staff_count" name="staff_count" value="{{ old('staff_count', 1) }}" min="1" max="10" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-600" required>
            ...
            const html = `
                <div class="subtype-item p-4 border border-gray-200 rounded-lg bg-gray-50">
                    <div class="flex justify-between items-center mb-3">
                        <label class="text-sm font-medium text-gray-700">Subtype #${id + 1}</label>
